...
@try_export
def export_torchscript(model, im, file, optimize, prefix='TorchScript:'):
...
@try_export
def export_openvino(model, file, half, prefix='OpenVINO:'):
...
@try_export
def export_coreml(model, im, file, int8, half, prefix='CoreML:'):
...
@try_export
def export_engine(model, im, file, half, dynamic, simplify, workspace=4, verbose=False, prefix='TensorRT:'):
...
@try_export
def export_tflite(keras_model, im, file, int8, data, nms, agnostic_nms, prefix='TensorFlow Lite:'):
...
@try_export
def export_edgetpu(file, prefix='Edge TPU:'):
...
@try_export
def export_tfjs(file, prefix='TensorFlow.js:'):
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default='/home/wgzhong/card-detection/models/yolov5s.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default='/home/wgzhong/card-detection/runs/exp/weights/best.pt', help='model.pt path(s)')
